...
public class AssetBundleTest : MonoBehaviour {
	...
	void LoadResources()
	{
		...
		Object preAtomBall = Resources.Load("test/pre_AtomBall");
		objectList.Add(Instantiate(preAtomBall) as GameObject);

		Object preSpikeBall = Resources.Load("test/pre_SpikeBall");
		objectList.Add(Instantiate(preSpikeBall) as GameObject);

		Object prePlayer = Resources.Load("player/pre_Player");
	...
	{
		OnAsyncTaskStart();
		StartCoroutine(reader.LoadAssetAsync("test", "pre_AtomBall", typeof(GameObject),
		...
		StartCoroutine(reader.LoadAssetAsync("test", "pre_SpikeBall", typeof(GameObject),
			delegate(Object asset)
			{
				objectList.Add(Instantiate(asset) as GameObject);
				OnLoadDone();
		...
		StartCoroutine(reader.LoadAssetAsync("player", "pre_Player", typeof(GameObject),
			delegate(Object asset)
			{
				objectList.Add(Instantiate(asset) as GameObject);
				OnLoadDone();
